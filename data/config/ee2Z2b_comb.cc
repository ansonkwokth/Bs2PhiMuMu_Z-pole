...
int main() {
    ...
    for (int iEvent = 0; iEvent < nEvent; ++iEvent) {
        ...
        for (int ipart = 0; ipart < event.size(); ipart++) {
            if (event[ipart].id() == 13) Flagtarget1 = 1;
        }
        for (int ipart = 0; ipart < event.size(); ipart++) {
            if (event[ipart].id() == -13) Flagtarget2 = 1;
        }
        for (int ipart = 0; ipart < event.size(); ipart++) {
            if (event[ipart].id() == 321) Flagtarget3 = 1;
        }
        for (int ipart = 0; ipart < event.size(); ipart++) {
            if (event[ipart].id() == -321) Flagtarget4 = 1;
        }
        if (Flagtarget1 == 1 && Flagtarget2 == 1 && Flagtarget3 == 1 && Flagtarget4 == 1) {
            Ntarget++;
